...
def test_type_hints():
    ...
    assert sig.return_annotation == bool, "❌ is_expired deveria retornar bool"
    print("✅ CacheEntry.is_expired tem type hint correto")
    
    sig = inspect.signature(CacheEntry.remaining_ttl)
    assert sig.return_annotation == int, "❌ remaining_ttl deveria retornar int"
...
async def test_integration():
    ...
    for i in range(5):
        ...
        if cached is None and i == 0:
            ...
            assert False, f"❌ Estado inesperado na request {i+1}"
    ...
    assert limiter_stats["total_clients"] == 1, f"❌ Deveria ter 1 cliente"
...
async def run_all_tests():
    ...
    try:
        ...
        print("="*70)
        print(f"✅ 7 testes executados com sucesso")
